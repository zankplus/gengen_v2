...
abstract class NameAssembly
{
	Random rng;
	Phonology p;
	
	abstract Name makeName();
}

//class ComplexAssembly extends NameAssembly
//{
//	// Flowchart nodes
...
//	 */
//	public ComplexAssembly(Phonology p)
//	{
//		this.p = p;
//		rng = p.rng;
//	
...
//				double simpleNucleusSimpleInterlude = prev.followers().countMembersOfLength(1) * 
//						p.counts[Phonology.SIMPLE_ONSETS];
//
//				double simpleNucleusEmptyInterlude = 0;
//				for (int i = 0; i < prev.followers().countMembersOfLength(1); i++)
//					if (prev.followers().getMembersOfLength(1).get(i).followers().countMembersOfLength(1) > 0)
//						simpleNucleusEmptyInterlude++;
//				
//				// chance of heavy rimes
//				double heavySimple = prev.followers().countMembersOfLength(1) * (p.counts[Phonology.COMPLEX_ONSETS] + 
...
//							p.counts[Phonology.COMPOUND_INTERLUDES];
//					heavyComplex *= prev.followers().countMembersOfLength(2);
//				}
//				
...
//			if (prev != null && prev.type == ConstituentType.NUCLEUS)
//			{
//				next = prev.followers().pickSimple();
...
//				simple *= prev.followers().countMembersOfLength(1);	
//				
//				// Log scale the count and multiply it by the inverse of the base diphthong chance
...
//					complex *= prev.followers().countMembersOfLength(2);
//					
...
//				else
//				{
//					next = prev.followers().pickComplex();;
//					pName *= complex / sum;
//					pName *= next.probability;
//				}
//			}
...
//		public Node nextNode()
//		{
//			double hiatusProminence = Math.log((prev.followers().countMembersOfLength(1) > 0 ? 1 : 0) + 1);
//			hiatusProminence *= (1 - p.baseMedialOnsetChance);
//			
...
//				Constituent nextOnset = next.followers().pick();
//				
...
//				double simpleNucleusSimpleCoda = 0;
//				if (p.baseTerminalCodaChance > 0)
//					simpleNucleusSimpleCoda = prev.followers().countMembersOfLength(1) * p.counts[Phonology.SIMPLE_CODAS];
//				
//				double simpleNucleusEmptyCoda = 0;
//				if (p.baseTerminalCodaChance < 1)
//					simpleNucleusEmptyCoda = prev.followers().countMembersOfLength(1);
//
//				// heavy rimes
//				double heavySimple = 0;
//				if (p.baseTerminalCodaChance > 0)
//					heavySimple = prev.followers().countMembersOfLength(1) * p.counts[Phonology.COMPLEX_CODAS];
//				
//				double heavyComplex = 0;
//				if (p.nuclei.maxLength() > 1)
//				{
...
//						
//						for (int j = 1; j <= coda.followers().maxLength(); j++)
//							for (Constituent onset : coda.followers().getMembersOfLength(j))
//							{
//								// Multiplied by onset probability
//								double interludeProb = onset.probability * coda.followers().getLengthProbability(j);
